 ...
 */
public class RequestsGenerator implements PbGenerator {

    private static String PACKAGE_NAME = "requests";
    ...
    private void generateReqeust(Model model) {
        model.services().forEach(this::generateReqeust);
    }

    private void generateReqeust(Service service) {
    ...
    private void generateRequest(Method method, Service service) {
        // Begin class
        String request = pbNames.getRequestClassName(method, service);
        ...
        for (int index = 0; index < inputParametersArray.length; index++) {
            generateRequestParameter(index, inputParametersArray[index]);
        }
    ...
    private void generateRequestParameter(int index, Parameter parameter) {
        // Get parameter name
